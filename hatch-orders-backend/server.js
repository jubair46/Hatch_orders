...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && pathname === '/admin') {
    if (!requireAuthOr401(req, res)) return;
    return fs.readFile(ADMIN_HTML, (err, data) => {
      if (err) { res.writeHead(500); return res.end('admin.html missing'); }
...
server.listen(PORT, () => console.log('Hatch Orders (no deps) running on port ' + PORT));
